...
Namespace RichEditBBCExporter
	Public Class BBCodeDocumentExporter
        ...
        Public ReadOnly Property Filter() As FileDialogFilter Implements IExporter(Of DocumentFormat, Boolean).Filter
            ...
            End Get
        End Property
        Public ReadOnly Property Format() As DocumentFormat Implements IExporter(Of DocumentFormat, Boolean).Format
            Get
        ...
        Public Function SaveDocument(ByVal documentModel As DocumentModel, ByVal stream As Stream, ByVal options As IExporterOptions) As Boolean Implements IExporter(Of DocumentFormat, Boolean).SaveDocument
            documentModel.RaiseBeforeExport(Format, options)

            Dim exporter As New BBCodeExporter(documentModel, CType(options, BBCodeDocumentExporterOptions))
	...
	Public Class BBCodeExporter
        ...
        Public Overloads Sub Export(ByVal outputStream As Stream)
		...
		Protected Overrides Sub ExportTextRun(ByVal run As TextRun)
			...
			If (Not hyperlinkExporting) Then
				...
				End If
				If run.ForeColor <> Color.Empty Then
					DocumentContentWriter.Write(String.Format("[color=#{0}]", run.ForeColor.Name.Substring(2)))
				End If
				If run.FontSize <> DocumentModel.DefaultCharacterProperties.FontSize Then
					DocumentContentWriter.Write(String.Format("[size={0}]", Math.Min(run.FontSize, 39)))
				End If
			End If
