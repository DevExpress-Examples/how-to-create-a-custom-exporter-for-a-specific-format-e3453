...
namespace RichEditBBCExporter {
    public class BBCodeDocumentExporter : IExporter<DocumentFormat, bool> {
        ...
        public bool SaveDocument(DocumentModel documentModel, Stream stream, IExporterOptions options) {
            ...
            BBCodeExporter exporter = new BBCodeExporter(documentModel, (BBCodeDocumentExporterOptions)options);
    ...
    public class BBCodeExporter : DocumentModelExporter {
        ...
        protected override void ExportTextRun(TextRun run) {
            ...
            if (!hyperlinkExporting) {
                ...
                if (run.ForeColor != Color.Empty)
                    DocumentContentWriter.Write(string.Format("[color=#{0}]", run.ForeColor.Name.Substring(2)));
                if (run.FontSize != DocumentModel.DefaultCharacterProperties.FontSize)
